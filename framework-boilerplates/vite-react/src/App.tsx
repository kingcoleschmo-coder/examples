...
function runSmokeTests() {
  try {
    ...
    for (const w of BRAND.deliveryWindows) {
      console.assert(typeof w.label === "string" && typeof w.time === "string", "delivery window must have label/time strings");
    }
    ...
    // Don’t crash the app if console.assert throws in some environments.
  }
}
...
export default function App() {
  ...
  const windowA = BRAND.deliveryWindows?.[0]?.label ?? "Lunch";
  const windowB = BRAND.deliveryWindows?.[1]?.label ?? "Dinner";
  const howText = `Select ${windowA} or ${windowB} delivery.`; // fixed template string
